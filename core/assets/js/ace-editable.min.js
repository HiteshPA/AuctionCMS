...
!function(a) {
	...
	a.fn.editableutils.inherit(b,a.fn.editabletypes.abstractinput),a.extend(b.prototype,{
		initImage:function(b,c) {
			...
			,c.image,b.image),this.name = this.options.image.name || "editable-image-input"}
	,render:function() {
		var a = this;
	...
	(window.jQuery),function(a) {
		"use strict";
		var b = function(c) {
			this.init("wysiwyg",c,b.defaults),this.options.wysiwyg = a.extend({
			}
		...
		(window.jQuery),function(a) {
			...
			,activate:function() {
				if(this.$input.is(":visible"))if(this.$input.focus(),a.fn.editableutils.setCursorPosition(this.$input.get(0),this.$input.val().length),this.nativeUI) {
					...
					for(var b = ["min","max","step"],c = 0;
						c<b.length;
						c++)b[c]in this.options.spinner && this.$input.attr(b[c],this.options.spinner[b[c]])}
				else{
					var d = parseInt(this.$input.val()),b = a.extend({
						value:d}
			...
			(window.jQuery),function(a) {
				...
				,activate:function() {
					if(this.$input.is(":visible"))if(this.$input.focus(),a.fn.editableutils.setCursorPosition(this.$input.get(0),this.$input.val().length),this.nativeUI) {
						...
						for(var b = ["min","max","step"],c = 0;
							c<b.length;
							c++)b[c]in this.options.slider && (this.$input[0][b[c]] = this.options.slider[b[c]]);
						...
						var e = this,f = parseInt(this.$input.val()),d = this.options.slider.width || 200,b = a.extend(this.options.slider,{
							value:f,slide:function(b,c) {
								...
								e.$input.val(d),null == c.handle.firstChild && a(c.handle).prepend("<div class='tooltip top in' style = 'display:none;
									top:-38px;
									left:-5px;
									'><div class='tooltip-arrow'></div><div class='tooltip-inner'></div></div>"),a(c.handle.firstChild).show().children().eq(1).text(d)}
						}
						);
				...
				,a.fn.editabletypes.abstractinput.defaults,{
					tpl:'<input type = "text" /><span class="inline ui-slider-green"><span class="slider-display"></span></span>',inputclass:"",slider:{
						min:1,max:100,step:1,range:"min"}
				}
				...
				(window.jQuery),function(a) {
					...
					,a.fn.editabletypes.abstractinput.defaults,{
						tpl:'<div class="input-group input-group-compact"><input type = "text" class="input-medium date" /><span class="input-group-addon"><i class="fa fa-calendar"></i></span></div>',date:{
							weekStart:0,startView:0,minViewMode:0}
					}
