...
!function(a) {
	...
	(function(a,b,c,d) {
		...
		var e = a.fn.dataTable,f = {
			version:"1.0.4-TableTools2",clients:{
			}
				,moviePath:"",nextId:1,$:function(a) {
	...
	,getDOMObjectPosition:function(a) {
		...
		for(""! == a.style.width && (b.width = a.style.width.replace("px","")),""! == a.style.height && (b.height = a.style.height.replace("px",""));
				a;
	...
	,Client:function(a) {
		...
		,this.id = f.nextId++,this.movieId = "ZeroClipboard_TableToolsMovie_"+this.id,f.register(this.id,this),a && this.glue(a)}
		};
		f.Client.prototype = {
			id:0,ready:!1,movie:null,clipText:"",fileName:"",action:"copy",handCursorEnabled:!0,cssEffects:!0,handlers:null,sized:!1,sheetName:"",glue:function(a,b) {
				...
				g.position = "absolute",g.left = "0px",g.top = "0px",g.width = e.width+"px",g.height = e.height+"px",g.zIndex = d,"undefined"! = typeof b && ""! == b && (this.div.title = b),0! == e.width && 0! == e.height && (this.sized = !0),this.domElement && (this.domElement.appendChild(this.div),this.div.innerHTML = this.getHTML(e.width,e.height).replace(/ & /g," & amp;
							"))}
			   ...
			   ,destroy:function() {
				   ...
				   ,receiveEvent:function(a,d) {
					   ...
					   switch(a = a.toString().toLowerCase().replace(/^on/,"")) {
						   case"load":if(this.movie = c.getElementById(this.movieId),!this.movie)return e = this,void setTimeout(function() {
							   ...
							   ,1);
								      ...
								      break;
						   case"mouseover":this.domElement && this.cssEffects& & this.recoverActive && this.domElement.addClass("active");
								   break;
						   ...
						   case"mouseup":this.domElement && this.cssEffects& & (this.domElement.removeClass("active"),this.recoverActive = !1)}
								 if(this.handlers[a])for(var f = 0,g = this.handlers[a].length;
										 g>f;
										 f++) {
				...
				,m = function(a,b) {
					for(var c = l(b),e = a.buttons.exportData(b.exportOptions),f = b.fieldBoundary,g = b.fieldSeparator,h = new RegExp(f,"g"),i = b.escapeChar! == d?b.escapeChar:"\\",j = function(a) {
						for(var b = "",c = 0,d = a.length;
							d>c;
							c++)c>0 && (b+ = g),b+ = f?f+(""+a[c]).replace(h,i+f)+f:a[c];
						return b}
						,k = b.header?j(e.header)+c:"",m = b.footer && e.footer?c+j(e.footer):"",n = [],o = 0,p = e.body.length;
						p>o;
						o++)n.push(j(e.body[o]));
					return{
						str:k+n.join(c)+m,rows:n.length}
				}
				,n = {
					available:function() {
						return f.hasFlash()}
						  ,init:function(a,b,c) {
							  ...
							  ,destroy:function(a,b,c) {
								  c._flash.destroy()}
							  ,fieldSeparator:",",fieldBoundary:'"',exportOptions:{
							  }
							  ,title:"*",filename:"*",extension:".csv",header:!0,footer:!1};
									...
									return a.i18n("buttons.excel","Excel")}
									  ,action:function(b,c,e,f) {
										  var g = "",j = f._flash,l = c.buttons.exportData(f.exportOptions),m = function(b) {
											  for(var c = [],e = 0,f = b.length;
												  f>e;
												  e++)null! == b[e] && b[e]! == d || (b[e] = ""),c.push("number" == typeof b[e] || b[e].match && a.trim(b[e]).match(/^-?\d+(\.\d+)?$/) && "0"! == b[e].charAt(0)?'<c t = "n"><v>'+b[e]+"</v></c>":'<c t = "inlineStr"><is><t>'+(b[e].replace?b[e].replace(/ & (?!amp;
																  )/g," & amp;
															  ").replace(/</g," & lt;
															  ").replace(/>/g," & gt;
															  ").replace(/[\x00-\x1F\x7F-\x9F]/g,""):b[e])+"</t></is></c>");
															  return"<row>"+c.join("")+"</row>"};
										  f.header && (g+ = m(l.header));
										  for(var n = 0,o = l.body.length;
											  o>n;
											  n++)g+ = m(l.body[n]);
							...
							,n,{
								className:"buttons-pdf buttons-flash",text:function(a) {
									return a.i18n("buttons.pdf","PDF")}
									  ,action:function(a,b,c,d) {
										  var e = d._flash,f = b.buttons.exportData(d.exportOptions),g = b.table().node().offsetWidth,i = b.columns(d.columns).indexes().map(function(a) {
											  return b.column(a).header().offsetWidth/g}
											  );
										  e.setAction("pdf"),e.setFileName(j(d)),k(e,JSON.stringify({
											  title:h(d,!1),message:d.message,colWidth:i.toArray(),orientation:d.orientation,size:d.pageSize,header:d.header?f.header:null,footer:d.footer?f.footer:null,body:f.body}
											  ))}
