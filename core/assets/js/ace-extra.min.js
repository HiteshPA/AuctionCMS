!function() {
	...
	,"vars"in window.ace || (window.ace.vars={
	...
	),ace.vars.very_old_ie = !("querySelector"in document.documentElement),ace.settings={
		saveState:function(a,b,c,d) {
			...
			l.test(b)?h = i?c:a[b]:a.hasAttribute(b)?h = i?c:a.getAttribute(b):i || (j = !0),j?delete f[b]:k.test(b)?(f.hasOwnProperty(b) || (f[b] = {
			}
			),d === !0?f[b][h] = 1:d === !1?f[b][h] = -1:f[b].className = h):f[b] = h,ace.data.set("state","id-"+e,JSON.stringify(f))}
	...
	,function() {
		var a = function() {
			...
			if(void 0! == f.style.animationName && (a = !0),a === !1)for(var g = 0;
					g<d.length;
					g++)if(void 0! == f.style[d[g]+"AnimationName"]) {
						e = d[g],b = e+"Animation",c = "-"+e.toLowerCase()+"-",a = !0;
						break}
			return a};
		if(ace.vars.animation = a(),ace.vars.animation) {
			var b = "@keyframes nodeInserted{
				from{
					outline-color:#fff}
			to{
				outline-color:#000}
		...
		document.addEventListener("animationstart",d,!1),document.addEventListener("MSAnimationStart",d,!1),document.addEventListener("webkitAnimationStart",d,!1)}
			...
			var e = function() {
				for(var a = document.querySelectorAll(".ace-save-state"),b = 0;
						b<a.length;
						b++)ace.settings.loadState(a[b])};
	...
	(),ace.data_storage = function(a,b) {
		...
		(1 == a || a === b || 0 == a) && "localStorage"in window && null! == window.localStorage?(d = ace.storage,e = 1):null == d && (2 == a || a === b) && "cookie"in document && null! == document.cookie && (d = ace.cookie,e = 2),this.set = function(a,b,f,g,h,i) {
			if(d)if(f === i)f = b,b = a,null == f?d.remove(c+b):1 == e?d.set(c+b,f):2 == e && d.set(c+b,f,ace.config.cookie_expiry,g || ace.config.cookie_path);
	...
	,ace.cookie = {
		get:function(a) {
			...
			if(d) {
				...
				return b = d.indexOf(";
						",c),-1 == b && (b = d.length),decodeURIComponent(d.substring(c+e.length,b))}
		}
		    ,set:function(a,b,c,d,e,f) {
			    var g = new Date;
			    "object" == typeof c && c.toGMTString?c = c.toGMTString():parseInt(c,10)?(g.setTime(g.getTime()+1e3*parseInt(c,10)),c = g.toGMTString()):c = "",document.cookie = a+" = "+encodeURIComponent(b)+(c?";
					    expires="+c:"")+(d?";
					    path = "+d:"")+(e?";
					    domain = "+e:"")+(f?";
					    secure":"")}
	...
	,ace.addClass=function(a,b) {
		for(var c = b.split(/\s+/),d = 0;
				d<c.length;
				d++)if(c[d].length>0 && !ace.hasClass(a,c[d])) {
					var e = a.className;
	...
	,ace.removeClass=function(a,b) {
		for(var c = b.split(/\s+/),d = 0;
				d<c.length;
				d++)c[d].length>0 && ace.replaceClass(a,c[d]);
